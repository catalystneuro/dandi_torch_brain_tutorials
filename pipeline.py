...
parser.add_argument("--interval_ref_time", type=str, default="start_time")
...
class Pipeline(BrainsetPipeline):
    ...
    def download(self, manifest_item):
        ...
        asset = [a for a in dandiset.get_assets() if manifest_item.filename in a.path][0]

        # Download the asset if not already present or if redownload is specified
        if filepath.exists() and not self.args.redownload:
            self.update_status("Skipped Downloading")
        else:
            asset.download(filepath=filepath)
            self.update_status(f"Downloaded {filepath.name}")
